    ...
    while True:
        ...
        if (client_frame != None) and (server_frame != None):
            ...
            if key_press == ord("q"):
                ...
                if kind == 'client':
                    cv2.imwrite(
                        os.path.join(os.getcwd(), 'calibration', 'client_preview', 'calib_img_{}.jpeg'.format(i)),
                        client_frame)
                elif kind == 'server':
                    cv2.imwrite(
                        os.path.join(os.getcwd(), 'calibration', 'server_preview', 'calib_img_{}.jpeg'.format(i)),
                        server_frame)

                elif kind == 'stereo':
                    cv2.imwrite(os.path.join(os.getcwd(), 'calibration', 'stereo_preview', 'client',
                                             'calib_img_client_{}.tiff'.format(i)), client_frame)
                    cv2.imwrite(os.path.join(os.getcwd(), 'calibration', 'stereo_preview', 'server',
                                             'calib_img_serv_{}.tiff'.format(i)), server_frame)
