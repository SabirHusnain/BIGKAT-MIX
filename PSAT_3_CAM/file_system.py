...
class FileOrderingSystem:
    ...
    def move_files_to_record(self):
        ...
        if os.path.join(base_path, server_video_fname) in all_files:
            shutil.move(os.path.join(base_path, server_video_fname),
                        os.path.join(self.video_directory, server_video_fname))

        if os.path.join(base_path, client_video_fname) in all_files:
            shutil.move(os.path.join(base_path, client_video_fname),
                        os.path.join(self.video_directory, client_video_fname))

        if os.path.join(base_path, server_timestamps_fname) in all_files:
            shutil.move(os.path.join(base_path, server_timestamps_fname),
                        os.path.join(self.recording_directory, server_timestamps_fname))

        if os.path.join(base_path, client_timestamps_fname) in all_files:
            shutil.move(os.path.join(base_path, client_timestamps_fname),
                        os.path.join(self.recording_directory, client_timestamps_fname))

            # Copy the calibration files over

        for cal in calibration_files:
            shutil.copy(os.path.join(base_path, 'calibration', cal),
                        self.calibration_directory)  # Move all files to a calibration directory folder
