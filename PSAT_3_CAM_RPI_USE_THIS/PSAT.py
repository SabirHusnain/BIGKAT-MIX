...
server_right_ip = '192.168.137.101'
server_left_ip = '192.168.137.102'
...
def load_demo_images():
    ...
    master_frame = cv2.imread(os.path.join(
        base_path, 'artwork', 'master_img.tif'))
    right_frame = cv2.imread(os.path.join(
        base_path, 'artwork', 'right_img.tif'))
    left_frame = cv2.imread(os.path.join(base_path, 'artwork', 'left_img.tif'))

    return left_frame, master_frame, right_frame
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def shutdown_event(self):
        ...
        reply = QtWidgets.QMessageBox.question(self, "Shutdown", shutdown_msg, QtWidgets.QMessageBox.Yes,
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            if self.camera_backend_live:
                ...
                self.shh_connection_right = shh_client(server_right_ip, 'pi',
                                                       'raspberry')  # Make a connection to the client
                ...
                self.shh_connection_right.shutdown_server_pi()

                self.shh_connection_left = shh_client(server_left_ip, 'pi',
                                                      'raspberry')  # Make a connection to the client
                self.shh_connection_left.shutdown_server_pi()
    ...
    def create_demographics_group(self):
        ...
        Marker_Num_label = QtWidgets.QLabel("Markers:")
        self.Marker_Num = MyTimeLineEdit(self)
        self.Marker_Num.set_text("3")
        ...
        options_grid.addWidget(Marker_Num_label, 1, 0, 1, 1)
        options_grid.addWidget(self.Marker_Num, 1, 1, 1, 1)
        options_grid.addWidget(Condition, 2, 0, 1, 1)
        options_grid.addWidget(self.ConditionStack, 2, 1, 1, 1)
        options_grid.addWidget(start_Rec, 3, 1, 1, 1)
    ...
    def create_camera_group(self):
        ...
        # Server Left Camera
        server_left_cam_container = QtWidgets.QWidget()
        server_left_cam_VLayout = QtWidgets.QVBoxLayout()

        server_left_cam_label = QtWidgets.QLabel("Left Camera")
        server_left_cam_label.setObjectName("labelLeft")

        self.label2 = QtWidgets.QLabel(self)  # Label for the server cam image

        # Server Right Camera
        server_right_cam_container = QtWidgets.QWidget()
        server_right_cam_VLayout = QtWidgets.QVBoxLayout()

        server_right_cam_label = QtWidgets.QLabel("Right Camera")
        server_right_cam_label.setObjectName("labelRight")

        self.label3 = QtWidgets.QLabel(self)  # Label for the server cam image

        # Client Camera
        client_cam_container = QtWidgets.QWidget()
        client_cam_VLayout = QtWidgets.QVBoxLayout()

        client_cam_label = QtWidgets.QLabel("Middle Camera")
        client_cam_label.setObjectName("labelClient")

        # Load temporal images

        # Camera feeds
        image_scale_factor = 0.20
        self.label1 = QtWidgets.QLabel(self)
        self.label1.setObjectName("camImage")
        self.label2 = QtWidgets.QLabel(self)
        self.label2.setObjectName("camImage")
        self.label3 = QtWidgets.QLabel(self)
        self.label3.setObjectName("camImage")

        f1, f2, f3 = load_demo_images()
        self.image_size = (
        ...
        f3 = cv2.resize(f3, self.image_size)
        ...
        self.image3 = QtGui.QImage(f3, f3.shape[1], f3.shape[0],
                                   f3.strides[0], QtGui.QImage.Format_RGB888)

        self.label1.setPixmap(QtGui.QPixmap.fromImage(self.image1))
        self.label2.setPixmap(QtGui.QPixmap.fromImage(self.image2))
        self.label3.setPixmap(QtGui.QPixmap.fromImage(self.image3))

        self.label1.setAlignment(QtCore.Qt.AlignCenter)
        self.label2.setAlignment(QtCore.Qt.AlignCenter)
        self.label3.setAlignment(QtCore.Qt.AlignCenter)

        # client_cam_VLayout.addWidget(client_cam_VLayout)
        ...
        server_left_cam_VLayout.addWidget(self.label2)
        server_left_cam_container.setLayout(server_left_cam_VLayout)

        server_right_cam_VLayout.addWidget(self.label3)
        server_right_cam_container.setLayout(server_right_cam_VLayout)
        ...
        camera_layout.addWidget(server_left_cam_container)
        camera_layout.addWidget(server_right_cam_container)
    ...
    def create_keyboard(self):
        ...
        self.keyboard.connect_to_buttons(self.Marker_Num.recieve_input)
    ...
    def find_participant(self):
        ...
        if self.participant_list_loaded:
            ...
            if N_entries == 0:
                ...
                part_markerNum = int(
                    self.participant_list.loc[self.current_entry_index]['Markers'])
                print(ID, part_forename, part_surname,
                      part_DOB, part_Gender, part_recTime, part_markerNum)
                ...
                self.Marker_Num.set_text(str(part_markerNum))
    ...
    def change_condition(self):
        ...
        if self.participant_list_loaded:
            ...
            part_markerNum = int(part_data_cond['Markers'].values[0])
    ...
    def process_file(self, rec_name, parallel=True):
        ...
        markers = pd.read_csv(os.path.join(
            rec_name, 'demographics.csv'), header=None, index_col=0, squeeze=True, names=['Value']).loc['MarkerNum']
        self.Marker_Num.set_text(markers)

        print("THE REC NAME\n{}".format(rec_name))
        client_markers, server_right_markers, server_left_markers = self.process_video_files(
            rec_name, parallel=parallel)  # Get the markers from each video file

        p_nan_points, PL_mid = self.process_marker_data(
            client_markers, server_right_markers, server_left_markers, rec_name)
    ...
    def process_video_files(self, rec_name, parallel=True):
        ...
        self.proc2 = backend.posturalProc(v_fname=os.path.join(
            rec_name, 'videos', 'testIR_server_right.h264'), calibration_dir=os.path.join(rec_name, 'calibration'), kind='server_right')
        self.proc3 = backend.posturalProc(v_fname=os.path.join(
            rec_name, 'videos', 'testIR_server_left.h264'), calibration_dir=os.path.join(rec_name, 'calibration'), kind='server_left')
        
        if parallel:
            # Process the markers (in parallel)?
            client_queue = multiprocessing.Queue()
            server_right_queue = multiprocessing.Queue()
            server_left_queue = multiprocessing.Queue()
            print("Starting IR marker processes")
            p1 = multiprocessing.Process(target=self.proc.get_ir_markers, kwargs={
                'n_markers': self.Marker_Num.value(), 'tval': 20, 'out_queue': client_queue})
            p2 = multiprocessing.Process(target=self.proc2.get_ir_markers, kwargs={
                'n_markers': self.Marker_Num.value(), 'tval': 2, 'out_queue': server_right_queue})
            p3 = multiprocessing.Process(target=self.proc3.get_ir_markers, kwargs={
                'n_markers': self.Marker_Num.value(), 'tval': 2, 'out_queue': server_left_queue})
            ...
            server_right_markers = server_right_queue.get()
            server_left_markers = server_left_queue.get()
            ...
            client_markers = self.proc.get_ir_markers(
                n_markers=self.Marker_Num.value(),tval=20)
            server_right_markers = self.proc2.get_ir_markers(
                n_markers=self.Marker_Num.value(),tval=2)
            server_left_markers = self.proc3.get_ir_markers(
                n_markers=self.Marker_Num.value(),tval=2)

        mark=min((len(client_markers),len(server_right_markers),len(server_left_markers)))
            
        return client_markers[0:mark], server_right_markers[0:mark], server_left_markers[0:mark]

    def process_marker_data(self, client_markers, server_right_markers, server_left_markers, rec_name):
        """Stereo triangulate the marker positions and then save everything to disk"""

        # Process markers
        proc_all_markers = ir_marker.markers2numpy(
            client_markers, n_markers=self.Marker_Num.value())
        proc2_all_markers = ir_marker.markers2numpy(
            server_right_markers, n_markers=self.Marker_Num.value())
        proc3_all_markers = ir_marker.markers2numpy(
            server_left_markers, n_markers=self.Marker_Num.value())
        ...
        proc3_all_markers_reshape = proc3_all_markers.reshape(
            proc3_all_markers.shape[0], np.prod(proc3_all_markers.shape[1:]))
        ...
        np.savetxt(os.path.join(rec_name, 'server_right_IRpoints.csv'), proc2_all_markers_reshape,
                   header=cam_marker_headers, delimiter=',', comments='', fmt='%1.4f')
        np.savetxt(os.path.join(rec_name, 'server_left_IRpoints.csv'), proc3_all_markers_reshape,
                   header=cam_marker_headers, delimiter=',', comments='', fmt='%1.4f')

        # Perform stereo triangulation
        try:
            stereo_right = backend.stereo_process(
                self.proc, self.proc2, kind='right', calibration_dir=os.path.join(rec_name, 'calibration'))
            stereo_left = backend.stereo_process(
                self.proc, self.proc3, kind='left', calibration_dir=os.path.join(rec_name, 'calibration'))
            stereo_left_right = backend.stereo_process(
                self.proc2, self.proc3, kind='left_right', calibration_dir=os.path.join(rec_name, 'calibration'))
            ...
            self.proc = backend.posturalProc(v_fname=os.path.join(
                ...
                kind='client')
            self.proc2 = backend.posturalProc(v_fname=os.path.join(
                rec_name, 'videos', 'testIR_server_right.h264'), calibration_dir=os.path.join(rec_name, 'calibration'),
                kind='server_right')
            self.proc3 = backend.posturalProc(v_fname=os.path.join(
                rec_name, 'videos', 'testIR_server_left.h264'), calibration_dir=os.path.join(rec_name, 'calibration'),
                kind='server_left')
            stereo_right = backend.stereo_process(
                self.proc, self.proc2, kind == 'right', calibration_dir=os.path.join(rec_name, 'calibration'))
            stereo_left = backend.stereo_process(
                self.proc, self.proc3, kind == 'left', calibration_dir=os.path.join(rec_name, 'calibration'))
            stereo_left_right = backend.stereo_process(
                self.proc3, self.proc2, kind == 'left_right', calibration_dir=os.path.join(rec_name, 'calibration'))
            
        
        markers_a=np.array([proc_all_markers[:,0,:], proc2_all_markers[:,0,:], proc3_all_markers[:,0,:]])
        markers_b=np.array([proc_all_markers[:,1,:], proc2_all_markers[:,1,:], proc3_all_markers[:,1,:]])
        markers_c=np.array([proc_all_markers[:,2,:], proc2_all_markers[:,2,:], proc3_all_markers[:,2,:]])
        z_a=stereo_right.relative_pose_estimation(markers_a)
        z_b=stereo_right.relative_pose_estimation(markers_b)
        z_c=stereo_right.relative_pose_estimation(markers_c)

        markers_3D=np.zeros((proc_all_markers.shape[0],3,3))
        markers_3D[:,0,:]=z_a.T
        markers_3D[:,1,:]=z_b.T
        markers_3D[:,2,:]=z_c.T
        markers_3D=stereo_right.kalman_smoother(markers_3D)
        newDistance = np.sqrt(np.sum(np.square(np.diff(markers_3D, axis=1)), axis=2)).squeeze()
        markers_3D = markers_3D.reshape(markers_3D.shape[0], np.prod(markers_3D.shape[1:]))
        headers = ['m{0}_x,m{0}_y,m{0}_z'.format(i) for i in range(int(markers_3D.shape[1] / 3))]
        headers = ",".join(headers)
        np.savetxt(os.path.join(rec_name, 'NewFiltered3D.csv'), markers_3D, header=headers, delimiter=',', comments='', fmt='%1.4f')
        np.savetxt(os.path.join(rec_name, 'NewDistance.csv'), newDistance, delimiter=',', comments='', fmt='%1.4f')
        

        markers3d_right = stereo_right.triangulate_all_get_PL(
            proc_all_markers, proc2_all_markers)
        markers3d_left = stereo_left.triangulate_all_get_PL(
            proc_all_markers, proc3_all_markers)
        markers3d_left_right = stereo_left_right.triangulate_all_get_PL(
            proc2_all_markers, proc3_all_markers)

        mark = min(markers3d_right.shape[0], markers3d_left.shape[0])
        
        markers3d_stereo = np.array([markers3d_right[:mark], markers3d_left[:mark]])
        markers3d_all = np.array([markers3d_right[:mark], markers3d_left[:mark],
                                 markers3d_left_right[:mark], np.mean(markers3d_stereo, axis=0)])

        for i, markers in enumerate(markers3d_all):
            ...
            markers3d_filt = stereo.kalman_smoother(markers)
            ...
            marker_mid_3d = np.sum(markers, axis=1) / 2.0
            ...
            p_nan_points = 100 * (np.isnan(markers)
                                  [:, 0, 0].sum() / markers.shape[0])
            # Prepare the marker data for file
            reshape3d = markers.reshape(markers.shape[0], np.prod(
                markers.shape[1:]))  # Markers 3d reshaped
            ...
            np.savetxt(os.path.join(rec_name, f'3d_unfiltered_{file_kind}.csv'), reshape3d,
                       header=headers, delimiter=',', comments='', fmt='%1.4f')
            np.savetxt(os.path.join(rec_name, f'3d_filtered_{file_kind}.csv'), filt_reshape3d,
                       header=headers, delimiter=',', comments='', fmt='%1.4f')
            np.savetxt(os.path.join(rec_name, f'distance_between_LEDs_{file_kind}.csv'),
                       distance_between_leds_filt, header=headers, delimiter=',', comments='', fmt='%1.4f')

            np.savetxt(os.path.join(rec_name, f"mid3D_unfiltered_{file_kind}.csv"), marker_mid_3d,
                       header='x,y,z', delimiter=',', comments='', fmt='%1.4f')
            np.savetxt(os.path.join(rec_name, f"mid3D_filtered_{file_kind}.csv"), marker_mid_3d_filt,
                       header='x,y,z', delimiter=',', comments='', fmt='%1.4f')
            ...
            summary.to_csv(os.path.join(rec_name, f'summary_{file_kind}.csv'))
    ...
    def preview_triangulate(self, img1, img2, img3):
        """Report the position of any markers visable in the preview"""

        ir1 = ir_marker.find_ir_markers(img1, n_markers=self.Marker_Num.value(), it=0, tval=150, last_markers=None,
                                        plot_loc=None)
        ir2 = ir_marker.find_ir_markers(img2, n_markers=self.Marker_Num.value(), it=0, tval=150, last_markers=None,
                                        plot_loc=None)
        ir3 = ir_marker.find_ir_markers(img3, n_markers=self.Marker_Num.value(), it=0, tval=150, last_markers=None,
                                        plot_loc=None)
        ...
        if ir3 != None:
            for mark in ir3:
                #                 print(mark['pos'], mark['radius'])
                cv2.circle(img3, (int(mark['pos'][0]), int(mark['pos'][1])), int(
                    mark['radius']), (255, 0, 0), 10)

        # 3d marker tracking
        if (ir1 != None) and (ir2 != None) and (ir3 != None):
            ir1 = ir_marker.markers2numpy(
                [ir1], n_markers=self.Marker_Num.value())  # Pass as list
            ir2 = ir_marker.markers2numpy(
                [ir2], n_markers=self.Marker_Num.value())
            ir3 = ir_marker.markers2numpy(
                [ir3], n_markers=self.Marker_Num.value())

            markers3d = self.stereo.triangulate_all_get_PL(
                ir1, ir2, ir3).squeeze()  # Get the marker positions in 3d space
    ...
    def start_preview(self):
        ...
        serverRightPreviewProc = backend.posturalProc(
            kind='server_right_preview')
        serverLeftPreviewProc = backend.posturalProc(
            kind='server_left_preview')

        self.stereo = backend.stereo_process(
            clientPreviewProc, serverRightPreviewProc, serverLeftPreviewProc)
        ...
        while self.preview_live:
            ...
            f2, f3 = self.backend_camera.TCP_client_poll_videoPreview()

            # Draw markers to screen and trinagulate
            if self.preview_options_triangulate.isChecked() and (f1 != None) and (f2 != None) and (f3 != None):
                f1, f2, f3 = self.preview_triangulate(f1, f2, f3)

            if (f1 == 'DEAD') or (f2 == "DEAD") or (f3 == "DEAD"):
                print("DEADDDDD")
                break
            ...
            try:
                ...
                if np.any(f3):
                    self.render_preview(f3, self.label3)
            except:
                pass
            # Update the images
    ...
    def start_recording(self):
        ...
        if self.storage_loc == '':
            try:
                self.storage_loc = os.listdir('/media/pi')[0]
                storage_msgBox = QtWidgets.QMessageBox()
                storage_msgBox.setText(
                    f"""{self.storage_loc} has been selected as default. To change -> Set Save Directory""")
        ...
        self.backend_camera.TCP_client_start_UDP(t, os.path.join(
            self.storage_loc, 'testIR.h264'))  # Starts the video recording
    ...
    def check_timestamp_error(self):
        ...
        time_server_right = np.loadtxt(
            "timestamps_server_right.csv", delimiter=',')
        time_server_left = np.loadtxt(
            "timestamps_server_left.csv", delimiter=',')

        time_client = time_client[:min(
            time_client.shape[0], time_server_right.shape[0], time_server_left.shape[0])]

        time_server_right = time_server_right[:min(
            time_client.shape[0], time_server_right.shape[0], time_server_left.shape[0])]

        time_server_left = time_server_left[:min(
            time_client.shape[0], time_server_right.shape[0], time_server_left.shape[0])]

        time_client[time_client < 0] = np.NaN
        time_server_right[time_server_right < 0] = np.NaN
        time_server_left[time_server_left < 0] = np.NaN

        if np.any(np.abs(time_client - time_server_right) > 5) or np.any(np.abs(time_client - time_server_left) > 5):
            return True
        else:
            return False
    ...
    def get_demographics(self):
        ...
        demographics = {'forename': self.Forename_edit.text(), 'surname': self.Surname_edit.text(),
                        ...
                        'MarkerNum': self.Marker_Num.value()}
