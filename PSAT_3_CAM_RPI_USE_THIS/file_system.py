...
class FileOrderingSystem:
    ...
    def check_recorded_dir_exists(self):
        ...
        if not recorded_dir_exists:
            ...
            #subprocess.run(('sudo','mkdir','-p',self.recorded_data_dir))
    ...
    def find_participant_dir(self, demographics):
        ...
        if ID == '':
            self.participant_dir = os.path.join(self.recorded_data_dir,
                                                'DEBUG')  # If no demographics has been entered save the data in the folder DEBUG
        ...
        if not part_dir_exists:
            #subprocess.run(('sudo','mkdir','-p',self.participant_dir))
    ...
    def find_participant_recording(self):
        ...
        self.recording_directory = os.path.join(self.participant_dir,
                                                'Record_{}'.format(new_record_num))  # A directory for a specific record
        #subprocess.run(('sudo','mkdir','-p',self.recording_directory))
    ...
    def prep_record_directory(self):
        ...
        if not os.path.isdir(self.video_directory):
            #subprocess.run(('sudo','mkdir','-p',self.video_directory))
        ...
        if not os.path.isdir(self.calibration_directory):
            #subprocess.run(('sudo','mkdir','-p',self.calibration_directory))
    ...
    def move_files_to_record(self):
        ...
        server_right_video_fname = 'testIR_server_right.h264'
        server_left_video_fname = 'testIR_server_left.h264'
        client_video_fname = 'testIR.h264'

        client_timestamps_fname = 'time_stamps.csv'
        server_right_timestamps_fname = 'timestamps_server_right.csv'
        server_left_timestamps_fname = 'timestamps_server_left.csv'

        calibration_files = ['client_camera_calib_params.pkl', 'server_camera_calib_params.pkl',
                             'stereo_camera_calib_params.pkl']

        if os.path.join(base_path, server_right_video_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, server_video_fname),os.path.join(self.video_directory, server_video_fname)))
            shutil.move(os.path.join(base_path, server_right_video_fname),os.path.join(self.video_directory, server_right_video_fname))

        if os.path.join(base_path, server_right_video_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, server_video_fname),os.path.join(self.video_directory, server_video_fname)))
            shutil.move(os.path.join(base_path, server_left_video_fname),os.path.join(self.video_directory, server_left_video_fname))

        if os.path.join(base_path, client_video_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, client_video_fname),os.path.join(self.video_directory, client_video_fname)))
            shutil.move(os.path.join(base_path, client_video_fname),os.path.join(self.video_directory, client_video_fname))

        if os.path.join(base_path, server_right_timestamps_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, server_timestamps_fname),os.path.join(self.recording_directory, server_timestamps_fname)))
            shutil.move(os.path.join(base_path, server_right_timestamps_fname),os.path.join(self.recording_directory, server_right_timestamps_fname))

        if os.path.join(base_path, server_right_timestamps_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, server_timestamps_fname),os.path.join(self.recording_directory, server_timestamps_fname)))
            shutil.move(os.path.join(base_path, server_left_timestamps_fname),os.path.join(self.recording_directory, server_left_timestamps_fname))

        if os.path.join(base_path, client_timestamps_fname) in all_files:
            #subprocess.run(('sudo','mv',os.path.join(base_path, client_timestamps_fname),os.path.join(self.recording_directory, client_timestamps_fname)))
        ...
        for cal in calibration_files:
            #subprocess.run(('sudo','cp',os.path.join(base_path, 'calibration', cal),self.calibration_directory))
    ...
    def check_record_process_status(self, record_fname=None):
        ...
        expected_videos = ['testIR.h264', 'testIR_server_right.h264', 'testIR_server_left.h264']
