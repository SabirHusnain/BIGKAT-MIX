...
def get_ir_marker_process(markers, jobs, results):
    ...
    while True:
        ...
        if isinstance(in_job, str):
            ...
            ir = find_ir_markers(j, n_markers=markers, it=ID, tval=150)
...
def find_ir_markers(frame, n_markers, it=0, tval=150, last_markers=None, last_visible_markers=None, plot_loc=None):
    ...
    if plot_loc == None:
        ...
        dirs = [".\\markers\\found\\client\\", ".\\markers\\found\\server_right\\", ".\\markers\\found\\server_left\\",
                ".\\markers\\not_found\\client\\", ".\\markers\\not_found\\server_right\\", ".\\markers\\not_found\\server_left\\"]
    ...
    if last_markers == None:
        # Initial guess
        ir = find_ir_markers_nofilt(frame, n_markers=n_markers, it=it, tval=tval, gamma=None, plot_ax=None)
        ...
        if (y_min_floor == y_max_floor) or (x_min_floor == x_max_floor):
            ...
            ir = find_ir_markers_nofilt(
                frame_new,n_markers=n_markers, it=it, tval=tval, gamma=None, plot_ax=None)

        # If no markers found run it again with the whole image
        if ir == None:

            ir = find_ir_markers_nofilt(
                frame,n_markers=n_markers, it=it, tval=tval, gamma=None, plot_ax=None)
    ...
    if ir != None:
        ...
        if False in rads_max:
            ...
            ir = find_ir_markers_nofilt(
                frame, n_markers=n_markers, it=it, tval=tval, gamma=5, plot_ax=None)
...
def find_ir_markers_nofilt(frame, n_markers, it=0, tval=150, gamma=None, plot_ax=None):
...
def order_ir_markers(markers, last_visible_markers, offset_angle=-35):
...
def markers2numpy(all_markers, n_markers):
